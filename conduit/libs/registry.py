...
class Registry:
    ...
    def uploadFile(self,nexu_url,path):
        ...
        response = requests.post('{0}/service/rest/v1/components?repository=omef-raw-repo'.format(nexu_url), files=files,auth=(self.username, self.password),verify=False)
        return response
    def send(self, path,image_name=None, method="GET"):
        if  image_name is not None:
            token = self.get_token(image_name)
            self.HEADERS = {'Authorization': "Bearer {}".format(token)}
            return requests.get("{0}{1}".format(self.hostname, path),
    ...
    def fetch_versions(self, image_name):
        #token = self.get_token(image_name)
        #h = {'Authorization': "Bearer {}".format(token)}
        result = self.send("/v2/library/{0}/tags/list".format(image_name),image_name)
        #r = requests.get('{}/v2/library/{}/tags/list'.format(self.hostname, image_name),
        #             headers=h)
